...
df <- read_delim("data/NosAutoanalizamos_TD2019 - Hoja 1 - Clean.tsv", 
    delim = "\t", escape_double = FALSE, 
    trim_ws = TRUE)
```

```{r}
df <- df %>% mutate(across(ends_with('.cat'), as.factor))
```
...
# Para observar las otras variables cambiamos la columna de dentro del desc

df %>% arrange(desc(ALG.num)) %>% View()
```
...
df %>% summarise(across(ends_with('.num'), .fns =list(Mean = ~mean(.,na.rm=T),
                                               STD = ~sd(.,na.rm=T),
                                               Median = ~median(.,na.rm=T),
                                               IQR = ~IQR(.,na.rm=T)))) %>% 
  pivot_longer(cols=names(.),names_to='Name', values_to='Value') %>% 
  separate(Name,into=c('Asignatura','Estadistico'), sep='.num_') %>% 
...
ggplot(df, aes(x = ALG.num)) + 
  geom_histogram(aes(y=after_stat(density))) +
  geom_density(color= 'blue') + 
  geom_vline(xintercept = mean(df$ALG.num,na.rm=T), color = 'red')
```
...
qqPlot(df$ANM.num, 
       main = "Q-Q Plot: Notas de ANM vs. Distribución Normal")
```
...
tc <- table(df$ALG.cat,df$ANM.cat)
tc

mosaicplot(tc, xlab = 'ALG', ylab = 'ANM', color = rainbow(length(levels(df$ALG.cat))))
```
...
df_num <- df %>% select(ends_with('.num'))

matriz_cov <- cov(df_num, use = "pairwise.complete.obs")
matriz_corr_Pearson <- cor(df_num,method = c("pearson"), use = "pairwise.complete.obs")
matriz_corr_Spearman <- cor(df_num,method = c("spearman"), use = "pairwise.complete.obs")
...
df_num_2 <- df_num %>% mutate(ALG.num = (ALG.num)**0.5)

matriz_cov_2 <- cov(df_num_2, use = "pairwise.complete.obs")
matriz_corr_Pearson_2 <- cor(df_num_2,method = c("pearson"), use = "pairwise.complete.obs")
matriz_corr_Spearman_2 <- cor(df_num_2,method = c("spearman"), use = "pairwise.complete.obs")

matriz_cov_2

matriz_corr_Pearson_2

matriz_corr_Spearman_2
```
...
ggpairs(matriz_corr_Pearson)

ggpairs(matriz_corr_Spearman)
```
...
df_new <- df %>% select(ALG.cat,ANM.cat) %>% na.omit()

cont <- table(df_new$ALG.cat,df_new$ANM.cat)

chisq.test(cont, correct=F)
```
Como p_value = 2.04 10^(-7) concluimos que las variables ALG.cat y ANM.cat son dependientes.

```{r Solucion_Ej9b, include=TRUE, echo=TRUE, eval=TRUE, results=TRUE}
df_new2 <- df %>% select(ALG.cat,Sex) %>% na.omit() %>% mutate(Sex = factor(Sex))

cont2 <- table(df_new2$ALG.cat,df_new2$Sex)

chisq.test(cont2, correct=F)
```
Como p_value = 0.8998 concluimos que las variables ALG.cat y Sex son independientes.
